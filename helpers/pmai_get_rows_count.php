	...
	function pmai_get_rows_count( $field, $sub_field, $i, $parentRepeater ){
        ...
        switch ($sub_field['type']){
            ...
            case 'link':
                ...
                $field_keys = array('title', 'url', 'target');
                
                foreach ( $field_keys as $field_key){
                    if ( ! empty($parentRepeater) ) {
                        $parent_entries = explode($parentRepeater['delimiter'], $sub_field['values'][$field_key][$i]);
                        $entries = explode($field['is_variable'], $parent_entries[$parentRepeater['row']]);
                    }
                    else {
                        $entries = explode($field['is_variable'], $sub_field['values'][$field_key][$i]);
                    }
