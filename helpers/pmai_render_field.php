...
if ( ! function_exists('pmai_render_field')){
	...
	function pmai_render_field($field, $post = array(), $field_name = ""){
		...
		<div class="field field_type-<?php echo $field['type'];?> field_key-<?php echo $field['key'];?>">			
			<p class="label"><label><?php echo (in_array($field['type'], array('message', 'tab', 'accordion'))) ? $field['type'] : ((empty($field['label']) ? '' : $field['label']));?></label></p>
				...
				<?php switch ($field['type']) {
					...
					case 'link':
						...
						<div class="input">
							...
							<input type="text" placeholder="" value="<?php echo esc_attr( $current_field['title'] );?>" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][title]" class="text widefat rad4"/>
						</div>
						<div class="input">
							<label><?php _e("URL"); ?></label>
							<input type="text" placeholder="" value="<?php echo esc_attr( $current_field['url'] );?>" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][url]" class="text widefat rad4"/>
							<a href="#help" class="wpallimport-help" title="<?php _e('Use external URL or post ID, slug or title to link to that post.', 'wp_all_import_acf_add_on'); ?>" style="top:0;">?</a>

						</div>
						<div class="input">
							<label><?php _e("Target"); ?></label>
							<input type="text" placeholder="" value="<?php echo esc_attr( $current_field['target'] );?>" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][target]" class="text widefat rad4"/>
						</div>
						<?php
						break;
									...
									<div class="wpallimport-collapsed-content-inner">											
										...
										<div class="input">
											<div class="form-field wpallimport-radio-field wpallimport-realhomes_addonaddress_geocode_address_no_key">						
												<input type="radio" id="<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_no_key" <?php if (empty($current_field['address_geocode']) or esc_attr( $current_field['address_geocode'] ) == 'address_no_key'):?>checked="checked"<?php endif;?> value="address_no_key" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][address_geocode]" class="switcher">
												<label for="<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_no_key" class="chooser_label">No API Key</label>
												<a style="position: relative; top: -2px;" class="wpallimport-help" href="#help" original-title="Limited number of requests.">?</a>
											</div>
											<div class="form-field wpallimport-radio-field wpallimport-<?php echo $field_name; ?>_geocode_address_google_developers">
												<input type="radio" id="<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_google_developers" value="address_google_developers" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][address_geocode]" class="switcher" <?php if (esc_attr( $current_field['address_geocode'] ) == 'address_google_developers'):?>checked="checked"<?php endif;?> >
												<label for="<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_google_developers" class="chooser_label">Google Developers API Key - <a href="https://developers.google.com/maps/documentation/geocoding/#api_key">Get free API key</a></label>
												...
												<div class="switcher-target-<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_google_developers" style="display: none;">
													<div class="input sub_input">
														<label for="<?php echo $field_name; ?>_google_developers_api_key">API Key</label>
														<div class="input">
															<input type="text" style="width:100%;" value="<?php echo esc_attr( $current_field['address_google_developers_api_key'] );?>" id="<?php echo $field_name; ?>_google_developers_api_key" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][address_google_developers_api_key]">
														</div>
													</div>
												</div>										
											</div>
											<div class="form-field wpallimport-radio-field wpallimport-<?php echo $field_name; ?>_geocode_address_google_for_work">
												<input type="radio" id="<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_google_for_work" value="address_google_for_work" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][address_geocode]" class="switcher" <?php if (esc_attr( $current_field['address_geocode'] ) == 'address_google_for_work'):?>checked="checked"<?php endif;?>>
												<label for="<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_google_for_work" class="chooser_label">Google for Work Client ID &amp; Digital Signature - <a href="https://developers.google.com/maps/documentation/business">Sign up for Google for Work</a></label>
												...
												<div class="switcher-target-<?php echo preg_replace("%[^a-zA-Z0-9\s]%", "", $field_name); ?>_geocode_address_google_for_work" style="display: none;">
													<div class="input sub_input">
														<label for="<?php echo $field_name; ?>_google_for_work_client_id">Google for Work Client ID</label>
														<div class="input">
															<input type="text" style="width:100%;" value="<?php echo esc_attr( $current_field['address_google_for_work_client_id'] );?>" id="<?php echo $field_name; ?>_google_for_work_client_id" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][address_google_for_work_client_id]">
														</div>
														<label for="<?php echo $field_name; ?>_google_for_work_digital_signature">Google for Work Digital Signature</label>
														<div class="input">
															<input type="text" style="width:100%;" value="<?php echo esc_attr( $current_field['address_google_for_work_digital_signature'] );?>" id="<?php echo $field_name; ?>_google_for_work_digital_signature" name="fields<?php echo $field_name; ?>[<?php echo $field['key'];?>][address_google_for_work_digital_signature]">
														</div>
													</div>
												</div>										
											</div>																													
											...
											<table class="widefat acf_input" style="border:none;">
												<tbody>
													...
													if ($acf and version_compare($acf->settings['version'], '5.0.0') >= 0){
														...
														if (! empty($parent_field_id) && is_numeric($parent_field_id))
														{
					...
					case 'group':
						...
						if ($acf and version_compare($acf->settings['version'], '5.6.0') >= 0){

							$groupID = empty($field['ID']) ? $field['id'] : $field['ID'];

							if (!empty($groupID)) {

								$sub_fields = get_posts(array('posts_per_page' => -1, 'post_type' => 'acf-field', 'post_parent' => $groupID, 'post_status' => 'publish'));
								...
								if ( ! empty($sub_fields) ){
									...
									foreach ($sub_fields as $key => $sub_field){
										...
										<tr class="field sub_field field_type-<?php echo $sub_fieldData['type'];?> field_key-<?php echo $sub_fieldData['key'];?>">
											<td>
												<div class="inner">
													<?php echo pmai_render_field($sub_fieldData, $post, $field_name . '[' . $field['key'] . ']'); ?>
												</div>
											</td>
										</tr>
										<?php
