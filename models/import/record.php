...
class PMAI_Import_Record extends PMAI_Model_Record {
    ...
    public function parse_field($field, $CurrentFieldXpath, $fieldPath = "", $xpath_suffix = "", $repeater_count_rows = 0, $inside_repeater = FALSE) {
        ...
        switch ($field['type']) {
            ...
            case 'location-field':
                $values = array();
                $keys = array('address', 'lat', 'lng', 'zoom', 'center_lat', 'center_lng');
                foreach ($keys as $key){
                    if ("" != $CurrentFieldXpath[$key]) {
                        $values[$key] = XmlImportParser::factory($xml, $cxpath, $CurrentFieldXpath[$key], $file)
                            ->parse();
                        $tmp_files[] = $file;
                    }
                    else {
                        $values[$key] = array_fill(0, $count_records, "");
                    }
                }
                ...
                switch ($CurrentFieldXpath['address_geocode']) {
                    case 'address_no_key':
                        ...
                        $values['api_key'] = array_fill(0, $count_records, "");
                        $values['client_id'] = array_fill(0, $count_records, "");
                        $values['signature'] = array_fill(0, $count_records, "");
                    ...
                    case 'address_google_developers':
                        ...
                        if ("" != $CurrentFieldXpath['address_google_developers_api_key']) {
                            $values['api_key'] = XmlImportParser::factory($xml, $cxpath, $CurrentFieldXpath['address_google_developers_api_key'], $file)
                                ->parse();
                            $tmp_files[] = $file;
                        }
                        else {
                    ...
                    case 'address_google_for_work':
                        ...
                        if ("" != $CurrentFieldXpath['address_google_for_work_client_id']) {
                            $values['client_id'] = XmlImportParser::factory($xml, $cxpath, $CurrentFieldXpath['address_google_for_work_client_id'], $file)
                                ->parse();
                            $tmp_files[] = $file;
                        }
                        else {
                        ...
                        if ("" != $CurrentFieldXpath['address_google_for_work_digital_signature']) {
                            $values['signature'] = XmlImportParser::factory($xml, $cxpath, $CurrentFieldXpath['address_google_for_work_digital_signature'], $file)
                                ->parse();
                            $tmp_files[] = $file;
                        }
                        else {
            ...
            case 'link':
                ...
                break;
            // field types with possibility for multiple values
            case 'radio':
            case 'true_false':
            ...
            case 'clone':
                ...
                if ($acf and version_compare($acf->settings['version'], '5.0.0') >= 0 and !empty($CurrentFieldXpath)) {
                    ...
                    foreach ($CurrentFieldXpath as $sub_field_key => $sub_field_xpath) {
                        ...
                        if ($sub_fieldData){
                            if ($sub_fieldData['type'] == 'flexible_content') {
                                // vars
                                $flex_fields = acf_get_fields($sub_fieldData);
                                // loop through layouts, sub fields and swap out the field key with the real field
                                foreach (array_keys($sub_fieldData['layouts']) as $fi) {
                                    // extract layout
                                    $layout = acf_extract_var($sub_fieldData['layouts'], $fi);
                                    // append sub fields
                                    ...
                                    // append back to layouts
                                    $sub_fieldData['layouts'][$fi] = $layout;
                                }
                            }
            ...
            case 'group':
                ...
                if ($acf and version_compare($acf->settings['version'], '5.6.0') >= 0 and !empty($CurrentFieldXpath)) {
    ...
    public function import($importData = array()) { //$pid, $i, $import, $articleData, $xml, $is_cron = false, $xpath_prefix = ""
        ...
        $this->import_type = $importData['import']->options['custom_type'];

        $cxpath = $importData['xpath_prefix'] . $importData['import']->xpath;

        if (!empty($this->data)) {
            foreach ((array) $this->data as $key => $field) {
                $this->import_field($importData['pid'], $importData['i'], $key, $field);
            }
        }
    }
    ...
    public function import_field($pid, $i, $key, $field, $fieldContainerName = "", $parentRepeater = array()) {
        ...
        switch ($field['type']) {
            ...
            case 'date_picker':
                ...
                $this->update_post_meta($pid, $fieldContainerName . $field['name'], $v);
                break;
            case 'date_time_picker':
                $time = strtotime($field['values'][$i]);
                if (FALSE === $time) {
                    ...
                    $v = ($version and version_compare($version, '5.0.0') >= 0) ? date('Y-m-d H:i:s', $time) : $time;
                }
                $this->update_post_meta($pid, $fieldContainerName . $field['name'], $v);
                break;
            case 'google_map_extended':
            ...
            case 'link':
                ...
                if ( ! empty($field['values']['url'][$i]) && ! preg_match('%^https?://%i', $field['values']['url'][$i]) ){
                    $relationID = $field['values']['url'][$i];
                    if ( ! is_numeric($relationID) ){
                        $sql = "SELECT * FROM {$this->wpdb->posts} WHERE post_type != %s AND ( post_title = %s OR post_name = %s )";
                        $relation = $this->wpdb->get_row($this->wpdb->prepare($sql, 'revision', $relationID, sanitize_title_for_query($relationID)));
                        if ($relation){
                            $relationID = $relation->ID;
                        }
                    }
                    $field['values']['url'][$i] = get_permalink($relationID);
                }
                $this->update_post_meta($pid, $fieldContainerName . $field['name'], array(
                    'title' => $field['values']['title'][$i],
                    'url' => $field['values']['url'][$i],
                    'target' => $field['values']['target'][$i]
                ));
                break;
            case 'location-field':
            ...
            case 'true_false':
                ...
                if ($field_post){
                    ...
                    if (!empty($field_post_options['save_custom'])){
                        $current_choice = (!empty($field['values'][$i]) and is_array($field['values'][$i])) ? $field['values'][$i] : array();
                ...
                if ("" != $field['values'][$i]) {
                    ...
                    if (!empty($entries) and is_array($entries)) {
                        foreach ($entries as $ev) {
                            $relation = false;
                            if (ctype_digit($ev)) {
                                $relation = $this->wpdb->get_row($this->wpdb->prepare("SELECT * FROM {$this->wpdb->posts} WHERE ID = %s", $ev));
                            }
                            if (empty($relation))
                            {
                                if (empty($field['post_type'])){
                                    $sql = "SELECT * FROM {$this->wpdb->posts} WHERE post_type != %s AND ( post_title = %s OR post_name = %s )";
                                    $relation = $this->wpdb->get_row($this->wpdb->prepare($sql, 'revision', $ev, sanitize_title_for_query($ev)));
                                }
                                else{
                                    $sql = "SELECT * FROM {$this->wpdb->posts} WHERE post_type IN ('%s') AND ( post_title = %s OR post_name = %s )";
                                    $relation = $this->wpdb->get_row($this->wpdb->prepare($sql, implode("','", $field['post_type']), $ev, sanitize_title_for_query($ev)));
                                }
                            }
                            if ($relation) {
            ...
            case 'group':
                if (!empty($field['values'])):
                    foreach ($field['values'] as $sub_field_key => $sub_field_data) {
                        $sub_field = $sub_field_data['values'];
                        $sub_field['name'] = $sub_field_data['name'];
                        $this->import_field($pid, $i, $sub_field_key, $sub_field, $fieldContainerName . $field['name'] . "_");
                    }
                    $this->update_post_meta($pid, $fieldContainerName . $field['name'], '');
                endif;
                break;
                            ...
                            if (!empty($row)) {
                                ...
                                if ($field['is_ignore_empties'] and $field['is_variable'] === FALSE or $field['is_variable']) {
                                    ...
                                    foreach ($row as $sub_field_key_check => $sub_field_check) {
                                        ...
                                        if ( in_array($sub_field_check['type'], array('google_map', 'link'))  ){
                                ...
                                if ($is_row_import_allowed) {
                                    if ($field['is_variable'] !== FALSE and $field['is_variable'] != '') {
                                        ...
                                        foreach ($row as $sub_field_key => $sub_field) {
                                            ...
                                            elseif(count($row) === 1 || empty($countCSVrows)){
                                                ...
                                                if ($sub_field['type'] !== 'repeater') {
                                                    switch ($sub_field['type']){
                                                        case 'taxonomy':
                                                            ...
                                                            break;
                                                        case 'link':
                                                            $sub_field_keys = array('title', 'url', 'target');
                                                            foreach ($sub_field_keys as $sub_field_key){
                                                                if (!empty($parentRepeater)) {
                                                                    $parent_entries = explode($parentRepeater['delimiter'], $sub_field['values'][$sub_field_key][$i]);
                                                                    $sub_field['values'][$sub_field_key][$i] = isset($parent_entries[$parentRepeater['row']]) ? $parent_entries[$parentRepeater['row']] : '';
                                                                }
                                                                $entries = explode($field['is_variable'], $sub_field['values'][$sub_field_key][$i]);
                                                                $sub_field['values'][$sub_field_key][$i] = isset($entries[$k]) ? $entries[$k] : '';
                                                            }
                                                            break;
                                                        case 'google_map':
                                                            $sub_field_keys = array('lat', 'lng', 'address');
                                                            foreach ($sub_field_keys as $sub_field_key){
                                                                if (!empty($parentRepeater)) {
                                                                    $parent_entries = explode($parentRepeater['delimiter'], $sub_field['values'][$sub_field_key][$i]);
                                                                    $sub_field['values'][$sub_field_key][$i] = isset($parent_entries[$parentRepeater['row']]) ? $parent_entries[$parentRepeater['row']] : '';
                                                        ...
                                                        default:
                                                            ...
                                                            if (empty($sub_field['xpath']) && count($entries) === 1){
                                                                $entries = array_fill(0, $countCSVrows, $entries[0]);
                                                            }
    ...
    public function import_image($img_url, $pid, $logger, $search_in_gallery = FALSE) {
        ...
        $uploads = apply_filters('wp_all_import_images_uploads_dir', $uploads, $this->articleData, $this->parsing_data['xml'], $this->parsing_data['import']->id);
